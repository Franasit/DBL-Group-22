...
Purpose
-------
Build ADM2-level daily and annual flood indicators from two flood-mask
collections:
...
processed_data/flood_analysis/adm2/
    ...
    adm2_daily_flood_indicators_by_type.csv
    adm2_daily_flood_indicators_combined.csv

    adm2_annual_flood_indicators_by_type.csv
    adm2_annual_flood_indicators_combined.csv

    adm2_annual_flood_indicators_all.csv

    adm2_boundary_summary.csv
...
flood_days:
    Number of dates on which any flooded pixel was observed in the ADM2.

flood_event_count:
    Number of separate clusters of flood-observation days.

annual_flooded_percent:
    annual_unique_flooded_area_km2 / ADM2 area * 100.
...
# Used only to calculate polygon areas.
EQUAL_AREA_CRS = "EPSG:6933"

# A new flood event begins when the gap between observed flood days is
# larger than this.
EVENT_GAP_DAYS = 1

# Coordinate rounding used for flood-cell identity.
# 6 decimal places is far more precise than the ~0.002083 degree grid.
COORD_DECIMALS = 6
...
def extract_year_tile(path: Path):
    """
    Extract year and MODIS-style tile ID from filename.
...
def deduplicate_source_files(
    ...
    """
    Keep one file for each year + tile combination.

    Prefer the version without "(1)", "(2)", etc.
    """
...
def discover_flood_files() -> pd.DataFrame:
    """
    Discover recurring and unusual parquet files.

    Returns an inventory DataFrame.
    """
...
def identify_adm2_columns(
    gdf: gpd.GeoDataFrame,
):
    """
    Identify ADM2 ID and name columns.
    """
    ...
    if id_col is None:
        ...
        id_col = "GENERATED_ADM2_ID"

    if name_col is None:

        gdf = gdf.copy()

        gdf["GENERATED_ADM2_NAME"] = (
            gdf[id_col].astype(str)
        )

        name_col = "GENERATED_ADM2_NAME"
...
def load_adm2_boundary():
    """
    Load the explicitly defined South Sudan ADM2 boundary.
    """

    require_path(
        ADM2_BOUNDARY_FILE,
        "South Sudan ADM2 boundary",
    )

    print(
        f"\nLoading ADM2 boundary:\n"
        f"{ADM2_BOUNDARY_FILE}"
    )
    ...
    adm2, id_col, name_col = (
        identify_adm2_columns(
            adm2
        )
    )
    ...
    # Area calculation.
    adm2_equal_area = adm2.to_crs(
        EQUAL_AREA_CRS
    )

    adm2["adm2_area_km2"] = (
        adm2_equal_area.geometry.area
        / 1_000_000.0
    )
...
def read_flood_file(
    ...
    """
    Read and clean one flood parquet.
    """
...
def estimate_coordinate_spacing(
    ...
    """
    Estimate geographic grid spacing.

    Flood-mask data contain only flooded cells, so adjacent cells may be
    missing. The most common small coordinate difference is therefore used.
    """
...
def grid_cell_area_km2(
    ...
    """
    Approximate area of a geographic lat/lon grid cell.
...
def bbox_overlap(
    ...
    """
    Check whether flood and ADM2 bounding boxes overlap.
    """
...
def spatially_assign_adm2(
    df: pd.DataFrame,
    adm2: gpd.GeoDataFrame,
):
    """
    Assign flood cells to ADM2.
    """
...
def aggregate_joined_points(
    ...
    """
    Aggregate one joined flood dataset.
    ...
    # ------------------------------------------------------------------
    # Annual spatial footprint
    #
    # Same lat/lon flooded 20 times in one year counts only once here.
    # ------------------------------------------------------------------
...
def process_single_file(
    ...
    """
    Process one recurring OR unusual parquet file.
    """
    ...
    if not bbox_overlap(
        ...
        print(
            ...
            "does not overlap South Sudan ADM2."
        )
...
def process_combined_group(
    ...
    """
    Combine recurring + unusual files belonging to the same year/tile group.

    Duplicate:
        date + lat + lon

    observations are removed BEFORE aggregation.
    """
...
def count_flood_events(
    ...
    """
    Count clusters of consecutive/near-consecutive flood days.
    """
...
def merge_daily_parts(
    ...
    """
    Combine multiple daily tables.

    Different MODIS tiles may contribute to the same ADM2/day.
    """
...
def merge_annual_unique_parts(
    ...
    """
    Combine annual unique flood footprints from individual files.

    Tiles are distinct spatial units in this dataset.
    """
...
# ============================================================================
# 15. BUILD COMPLETE ADM2 × YEAR × TYPE GRID
# ============================================================================
...
def build_complete_grid(
    ...
    """
    Build every ADM2 × year × flood_type combination.

    Missing flood observations can then safely become zero.
    """
...
def build_annual_indicators(
    ...
    """
    Build final annual ADM2 flood indicator table.
    """
    ...
    # Due to point/grid representation, very small boundary-edge artefacts
    # are possible. Do not forcibly cap at 100 here; values above 100 are
    # useful as a diagnostic signal.
...
def main():
    ...
    # Combined processing
    #
    # Group files by year + tile, then merge recurring/unusual source files
    ...
    if not daily_type_parts:
        ...
        raise RuntimeError(
            ...
            "to ADM2.\n\n"
            "Check the printed coordinate bounds "
            "and ADM2 boundary bounds."
        )

    if not daily_combined_parts:

        raise RuntimeError(
            "\nNo combined flood data were produced."
        )
    ...
    # ----------------------------------------------------------------------
    # Merge annual spatial footprints
    # ----------------------------------------------------------------------
    ...
    # ----------------------------------------------------------------------
    # Annual recurring/unusual
    # ----------------------------------------------------------------------

    print_header(
        "BUILDING ANNUAL ADM2 INDICATORS"
    )
    ...
    if len(suspicious) > 0:
        ...
        print(
            "\nWARNING:"
            "\nSome ADM2-year observations have "
    ...
    print(
        ...
        "\n"
        "\nAfter ADM3 is complete, we will use:"
        "\n03_analyze_adm2_flood_trends.py"
        "\n"
        "\nto identify:"
        "\n  - persistent flood hotspots"
        "\n  - emerging hotspots"
        "\n  - declining hotspots"
        "\n  - recent flood expansion"
